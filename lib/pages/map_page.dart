...
class _MapPageState extends State<MapPage> {
  ...
  void initState() {
    ...
    getLocationsUpdates().then((_) => {
      getPolylinePoints().then((coordinates) => {
        generatesPolyLinesFromPoints(coordinates)
      }),
    });
  }
    ...
    return Scaffold(
      body: _currentP == null ? const Center(child: Text("Loading..."),) : GoogleMap(
       onMapCreated: ((GoogleMapController controller) => _mapController.complete(controller)),
        initialCameraPosition: CameraPosition(target: _pGooglePixel, zoom: 13),
        markers: {
  ...
  Future<List<LatLng>> getPolylinePoints() async {
    ...
    PolylineResult result = await polylinePoints.getRouteBetweenCoordinates(
        GOOGLE_API_KEY, PointLatLng(_pGooglePixel.latitude, _pGooglePixel.longitude),
  ...
  void generatesPolyLinesFromPoints(List<LatLng> polylinesCoodinates) async {
    ...
    Polyline polyline = Polyline(polylineId: id, color: Colors.black, points: polylinesCoodinates, width: 3);
    setState(() {
